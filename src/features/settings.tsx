...
import { Cloud, Database, Download, FileJson, FileText, HardDrive, Info, LogOut, ShieldCheck, Trash2 } from "lucide-react";
...
import { APP_RELEASE_DATE, APP_RELEASE_NOTES, APP_VERSION } from "@/app/version";
...
export function SettingsView() {
  ...
  return (
    <section className="view-page settings-page">
      ...
      <section className="settings-section"><header><div><h2>数据说明</h2><p>当前工作区的数据边界。</p></div><ShieldCheck size={19} /></header><div className="data-facts"><div><Database size={17} /><span><strong>{workspace.tasks.length + workspace.workEntries.length + workspace.learningEntries.length + workspace.thoughtEntries.length + workspace.dailyReviews.length}</strong><small>{syncMode === "cloud" ? "云端记录总数" : "本地记录总数"}</small></span></div><div>{syncMode === "cloud" ? <Cloud size={17} /> : <HardDrive size={17} />}<span><strong>{syncMode === "cloud" ? "Supabase" : "此浏览器"}</strong><small>当前存储位置</small></span></div></div></section>
      <section className="settings-section about-section"><header><div><h2>关于工作台</h2><p>当前部署版本与本版主要能力。</p></div><Info size={19} /></header><div className="version-summary"><div className="version-number"><small>版本</small><strong>v{APP_VERSION}</strong><span>更新于 {APP_RELEASE_DATE}</span></div><ul>{APP_RELEASE_NOTES.map((note) => <li key={note}>{note}</li>)}</ul></div></section>
